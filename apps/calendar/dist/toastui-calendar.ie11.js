...
function isUsingDST(targetDate, timezoneName) {
  ...
  var jan = new TZDate(targetDate.getFullYear(), 0, 1);
  var jul = new TZDate(targetDate.getFullYear(), 6, 1);
...
var datetime_MS_PER_DAY = 86400000;
...
function positionUIModels(start, end, matrices, iteratee) {
  var ymdListToRender = datetime_makeDateRange(start, end, datetime_MS_PER_DAY).map(function (date) {
...
function generateTimeArrayInRow(matrix) {
  var map = [];
  var maxColLen = Math.max.apply(Math, week_toConsumableArray(matrix.map(function (col) {
    return col.length;
  })));
  var cursor = [];
  var row;
  ...
  for (col = 1; col < maxColLen; col += 1) {
    ...
    while (event) {
      ...
      var _event$valueOf = event.valueOf(),
          goingDuration = _event$valueOf.goingDuration,
          comingDuration = _event$valueOf.comingDuration;
...
function week_hasCollision(arr, start, end) {
  ...
  var compare = array.compare.num.asc;
  var startStart = Math.abs(array.bsearch(arr, start, searchFunc(0), compare));
  var startEnd = Math.abs(array.bsearch(arr, start, searchFunc(1), compare));
  var endStart = Math.abs(array.bsearch(arr, end, searchFunc(0), compare));
  var endEnd = Math.abs(array.bsearch(arr, end, searchFunc(1), compare));
    ...
    matrix.forEach(function (row) {
      row.forEach(function (uiModel, col) {
        ...
        var _uiModel$model = uiModel.model,
            goingDuration = _uiModel$model.goingDuration,
            comingDuration = _uiModel$model.comingDuration;
...
function getExceedDate(maxCount, eventsInDateRange, range) {
  ...
  eventsInDateRange.forEach(function (matrix) {
    matrix.forEach(function (column) {
      column.forEach(function (uiModel) {
        if (!uiModel || uiModel.top < maxCount) {
          return;
        }

        var period = makeDateRange(uiModel.getStarts(), uiModel.getEnds(), MS_PER_DAY);
    ...
    return matrix.map(function (row) {
      if (row.length > visibleEventCount) {
        return row.filter(function (item) {
          return item.top < visibleEventCount;
        });
      }
...
var getDayGridEvents = function getDayGridEvents(row, calendarData, _ref3) {
...
function getWeekDates(renderDate, _ref5) {
  ...
  var renderDay = renderDate.getDay();
  var now = toStartOfDay(renderDate);
  var nowDay = now.getDay();
  var prevWeekCount = startDayOfWeek - WEEK_DAYS;
  return range_default()(startDayOfWeek, WEEK_DAYS + startDayOfWeek).reduce(function (acc, day) {
    var date = addDate(now, day - nowDay + (startDayOfWeek > renderDay ? prevWeekCount : 0));
...
var GridCells = compat_module_g(function GridCells(_ref) {
  ...
  return v(d, null, weekDates.map(function (cell, index) {
...
var DateSelector = compat_module_x(function DateSelector(_ref, ref) {
  var start = _ref.start,
      end = _ref.end,
      _ref$isAllday = _ref.isAllday,
      isAllday = _ref$isAllday === void 0 ? false : _ref$isAllday,
...
function EventFormPopup() {
  ...
  var _ref3 = popupParams !== null && popupParams !== void 0 ? popupParams : {},
      ...
      isAllday = _ref3$isAllday === void 0 ? false : _ref3$isAllday,
      _ref3$isPrivate = _ref3.isPrivate,
      isPrivate = _ref3$isPrivate === void 0 ? false : _ref3$isPrivate,
      _ref3$eventState = _ref3.eventState,
      eventState = _ref3$eventState === void 0 ? 'Busy' : _ref3$eventState,
...
var Panel = compat_module_x(function Panel(_ref2, ref) {
  var name = _ref2.name,
...
function day_Day() {
  ...
  var dayGridEvents = getDayGridEvents(days, calendarData, {
    narrowWeekend: narrowWeekend,
    hourStart: hourStart,
    hourEnd: hourEnd
  });
...
function AccumulatedGridSelection(_ref) {
  ...
  return v("div", {
    ...
    return gridSelectionData ? v(GridSelection, {
      type: "accumulated",
...
function Week() {
  ...
  var eventByPanel = F(function () {
    return getDayGridEvents(weekDates, calendarData, {
      narrowWeekend: narrowWeekend,
      hourStart: hourStart,
      hourEnd: hourEnd
    });
